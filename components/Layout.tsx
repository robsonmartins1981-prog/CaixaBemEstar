...
const Layout: React.FC<LayoutProps> = ({ children, activeTab, setActiveTab }) => {
  ...
  const menuItems = [
    ...
    { id: 'entries', label: 'CAIXA', icon: <Calculator size={20}/> },
    ...
    { id: 'import', label: 'PLANILHAS', icon: <FileUp size={20}/> },
  ];
  ...
  return (
    <div className="flex h-screen w-full bg-[#f1f5f9] overflow-hidden font-sans text-slate-800">
      ...
      {toast && (
        <div className={`fixed top-4 right-4 z-[250] px-6 py-3 rounded-xl shadow-2xl flex items-center gap-2 animate-in slide-in-from-top-4 border ${toast.type === 'success' ? 'bg-green-600 border-green-500 text-white' : 'bg-red-600 border-red-500 text-white'}`}>
      ...
      >
        <div className="h-full flex flex-col">
          <div className="p-5 bg-[#0f172a] flex items-center justify-between border-b border-slate-700">
            <div className="flex items-center gap-2">
              <div className="w-10 h-10 rounded bg-green-500 flex items-center justify-center font-black italic text-white text-sm shadow-lg">BE</div>
      ...
      <div className="flex-1 flex flex-col min-w-0 h-screen overflow-hidden">
        <header className="h-16 bg-white border-b border-slate-200 flex items-center justify-between px-6 shrink-0 z-50 no-print shadow-sm">
          <div className="flex items-center gap-4">
            <button onClick={() => setIsSidebarOpen(true)} className="lg:hidden p-2 text-slate-600 hover:bg-slate-50 rounded-lg transition-colors">
              <Menu size={24}/>
            </button>
            <h2 className="text-[11px] font-black text-slate-500 uppercase tracking-widest border-l-4 border-green-500 pl-3">

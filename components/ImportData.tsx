...
const ImportData: React.FC<{ onSuccess: () => void }> = () => {
  ...
  const handleExportData = (type: 'caixa' | 'contas') => {
    try {
      ...
      if (type === 'caixa') {
        headers = ["Data", "Turno/Caixa", "Dinheiro (R$)", "Pix (R$)", "Crédito (R$)", "Débito (R$)", "Sangria (R$)", "Líquido (R$)"];
        rows = db.getEntries().map(e => [
          ...
          e.debit, 
          e.sangria,
          (e.cash + e.pix + e.credit + e.debit - e.sangria)
        ]);
      } else {
  ...
  return (
    <div className="flex-1 flex flex-col gap-6 max-w-4xl mx-auto w-full py-10 px-4 h-full animate-in fade-in duration-500">
      
      <div className="text-center mb-8">
        <div className="inline-flex p-4 bg-blue-50 rounded-3xl mb-4 text-blue-600 shadow-inner">
          <HardDriveDownload size={40} />
        </div>
        <h2 className="text-3xl font-black text-slate-800 uppercase tracking-tighter">Central de Exportação</h2>
        <p className="text-xs font-bold text-slate-400 uppercase tracking-widest mt-2">Gere planilhas oficiais para contabilidade e gestão</p>
      </div>
